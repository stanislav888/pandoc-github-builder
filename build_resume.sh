...
HTML_TEMPLATE_FILE="${HTML_TEMPLATE_FILE:-${SCRIPT_DIR}/pandoc-templates/bootstrap-template.html}"
...
HTML_OUTPUT="${OUTPUT_FOLDER}/${FILE_BASENAME}_live.html"
ODT_OUTPUT="${OUTPUT_FOLDER}/${FILE_BASENAME}.odt"
DOCX_OUTPUT="${OUTPUT_FOLDER}/${FILE_BASENAME}.docx"
PREVIEW_HTML="/tmp/resume_bootstrap.html"

echo ""
echo "--- Generating HTML (live) ---"
pandoc "$INPUT_FILE" \
    -s \
    --toc \
    --toc-depth=3 \
